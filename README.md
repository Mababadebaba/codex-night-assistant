...
It lets Windows turn the screen off while keeping the computer awake, then monitors local Codex session activity and can shut down the PC after all Codex work appears idle or blocked by rate limits.
...
5. To shut down after global Codex completion, set:
   ...
   - `检测后延迟`: optional shutdown delay in seconds
6. Click `全局完成后关机`.
...
它允许系统自动息屏，但阻止电脑进入睡眠；同时监控本机 Codex 会话活动，在所有 Codex 任务看起来已经空闲，或者检测到额度/速率限制阻塞后，自动关闭电脑。
