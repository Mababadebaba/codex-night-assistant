...
- Detect recent Codex activity, active Codex-launched processes, pending tool calls, and rate-limit blocking signals.
- Shut down Windows after a configurable continuous idle period.
- Show live active turns, active calls, active processes, recent sessions, rate-limit status, and the current shutdown decision.
...
- Use Windows' force shutdown option after confirmation so unattended shutdown still works when the session is locked or the PC is in Modern Standby.
...
Recent session count is informational. Shutdown blocking is based on active Codex turns, active Codex-launched processes, pending tool calls, and the configured continuous idle time.

If any quota bucket is exhausted, or Codex logs an explicit rate-limit block, the tool treats that as quota exhaustion. It does not require multiple quotas to be exhausted at the same time. Quota percentages are read from the newest local Codex `token_count` log event.
...
- Confirmed shutdown uses Windows' force option because locked Windows sessions reject normal shutdown requests.
...
- 检测最近 Codex 活动、Codex 启动的仍在运行的进程、等待中的工具调用，以及额度/速率限制阻塞信号。
- 连续空闲达到设定时间后自动关机。
- 开启全局监控后，实时显示活跃回合、活跃调用、活跃进程、最近会话、额度状态和当前判断。
- 真正关机前必须弹出确认窗口；点否会取消自动关机，无人操作才会在倒计时结束后关机。
- 确认关机后使用 Windows 强制关机参数，避免锁屏或新型待机状态下普通关机命令被系统拒绝。
...
“最近会话”只是信息展示，不再作为阻止关机的活跃任务条件。真正阻止关机的是活跃回合、活跃进程、等待中的工具调用，以及你设置的连续空闲时间。

只要任意一个额度桶耗尽，或 Codex 日志明确出现额度/速率限制阻塞，程序就会视为额度已耗尽；不要求多个额度同时耗尽。额度百分比来自本机 Codex 日志里最新一条 `token_count` 记录。
...
- 工具不会强制点亮屏幕。
- 关闭窗口会释放防睡眠状态。
- 确认关机后会使用 Windows 强制关机参数，因为锁屏状态下普通关机命令会被系统拒绝。
