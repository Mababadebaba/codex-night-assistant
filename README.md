...
- Show live active calls, active processes, recent sessions, rate-limit status, and the current shutdown decision.
...
Recent session count is informational. Shutdown blocking is based on active Codex-launched processes, pending tool calls, and the configured continuous idle time.

If any quota bucket is exhausted, or Codex logs an explicit rate-limit block, the tool treats that as quota exhaustion. It does not require multiple quotas to be exhausted at the same time.
...
- 连续空闲达到设定时间后自动关机。
- 开启全局监控后，实时显示活跃调用、活跃进程、最近会话、额度状态和当前判断。
...
“最近会话”只是信息展示，不再作为阻止关机的活跃任务条件。真正阻止关机的是活跃进程、等待中的工具调用，以及你设置的连续空闲时间。

只要任意一个额度桶耗尽，或 Codex 日志明确出现额度/速率限制阻塞，程序就会视为额度已耗尽；不要求多个额度同时耗尽。
